...
function getPromptForLevel(level: number): string {
    ...
    switch (level) {
        case 1:
            levelPrompt = `Level 1: 아래 요청사항에 따라 이미지 변경해 준 뒤 생성된 이미지 데이터 전송해줘
1.인물 생김새 유지: 원본 인물 사진의 구조는 그대로 유지해야해. 
2.카메라거리는 원본 사진 보다 50% 멀리서 찍은 사진 처럼 축소해줘. 축소하면서 잘린 부분들을 채워넣어줘 그리고 카메라 앵글을 틀어야해
`;
            break;
        case 2:
            levelPrompt = `Level 2: 아래 요청사항에 따라 이미지 변경해 준 뒤 생성된 이미지 데이터 전송해줘
1.인물 생김새 유지: 원본 인물 사진의 구조는 그대로 유지해야해. 
2.카메라 거리는 원본 사진 보다 100% 가까이에서 확대해서 찍은 사진 처럼 만들어주면서 카메라 앵글을 틀어야해
`;
            break;
        case 3:
            levelPrompt = `Level 3: 아래 요청사항에 따라 이미지 변경해 준 뒤 생성된 이미지 데이터 전송해줘
1.인물 생김새 유지: 원본 인물 사진의 구조는 그대로 유지해야해. 
...
serve(async (req) => {
    ...
    try {
        ...
        if (errorMessage.includes("finishReason: SAFETY")) {
            return new Response(JSON.stringify({ error: "생성 요청이 Google의 안전 정책에 의해 차단되었습니다. 다른 이미지를 사용해 주세요." }), {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
                status: 400,
            });
        }
