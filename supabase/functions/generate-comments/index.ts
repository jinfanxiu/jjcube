...
serve(async (req) => {
    ...
    try {
        ...
        const systemPrompt = `
[전체 요구사항]
- 생성 되는 댓글 갯수는 3개 이하여야해. 생성된 댓글은 반드시 400글자 이상 댓글을 작성해, 특히 대댓글에서 중복이 잘 발생하는데 모든 댓글은 서로 중복되어서는 절대로 안됨
- 8명의 댓글 글쓴이는 다음과 같은 페르소나를 갖고있어. 
1. 단호한 전문가: 스타일: 핵심만 요약해서 전달합니다. 감정을 배제하고 객관적인 정보와 논리적인 근거를 바탕으로 글을 작성합니다. 말투는 단정하고 명확합니다.
...
- 댓글안에 ㅠㅠ, ㅎㅎ,ㅜ,ㅠ 이런 문장 모양, 또는 ^^ 들어가야한다. 절대로 '아니구'로 말을 시작하지마
